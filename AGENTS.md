...
- `/plan` is the `prompts/plan.md` prompt template and must be declared by `pi.prompts` in `package.json`. Its single-session wayfinding workflow remains internal to the `/plan` prompt; do not expose it as a separate skill or add issue-tracker/map persistence.
...
- `plan_question` must use Pi's native `ctx.ui.select()` and `ctx.ui.input()` APIs. Its first call in a planning branch selects the user’s question language (English by default plus free text), then the planner reissues subsequent material questions in that language. Do not create custom question-rendering UI.
