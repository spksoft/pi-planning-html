...
5. Call `plan_publish` once with the full plan. It validates the plan and writes the standalone HTML file under `docs/plan/` by default (or `.pi/planning.json`'s `artifact.directory`). Do not use generic write/edit tools to create the plan.
